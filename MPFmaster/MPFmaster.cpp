...
uint32_t GetSampleMSLength_PT(void* in)
{
    ...
    // Search for EA_PT_MAGIC
    int cursor = 0;
    while (cursor < size - 4) { // Ensure we don't go out of bounds
        if (*(uint32_t*)((size_t)in + cursor) == EA_PT_MAGIC) {
            break;
        }
        cursor++;
    }
    ...
    if (cursor >= size - 4) {
        // EA_PT_MAGIC not found, handle error
        // For example, you might throw an exception or return an error code
        return 0; // Return 0 for simplicity, handle error appropriately in your code
    }

    sf = (void*)((size_t)in + cursor + 4); // Move to the location after EA_PT_MAGIC
    ...
    // Continue processing data after EA_PT_MAGIC
